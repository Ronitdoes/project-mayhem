    ...
    <main className="min-h-screen bg-[#0d0415] text-purple-100 flex flex-col items-center justify-between p-4 md:p-8 font-mono relative overflow-hidden select-none">
      ...
      <header className="w-full max-w-6xl flex flex-col sm:flex-row justify-between items-center border-b border-purple-900/50 pb-4 mb-6 z-10">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">FILE PROFILE: CF-04-AB-2903 // THE FORTUNE TELLER</p>
        </div>
      ...
      <section className="flex flex-col items-center my-4 z-10 relative">
        ...
        <p className="text-[10px] text-purple-400/50 uppercase tracking-widest mt-2">Ambient whispers surrounding canvas...</p>
      </section>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-center border-t border-purple-950/40 pt-2">
                  <div className={`w-3 h-3 rounded-full border ${selectedCard === idx ? "bg-purple-500 border-purple-400" : "border-purple-900/60"}`} />
                </div>
              </div>
            );
          })}
      ...
      <section className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 items-start z-10">
        ...
        <div className="bg-zinc-950/60 border border-purple-950 p-5 rounded-xl flex flex-col justify-between h-full min-h-[220px]">
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-purple-400 mb-2">CONTRADICTION ANALYSIS DECK</h4>
            <p className="text-[10px] text-zinc-500 uppercase mb-4">Mark the targeted node containing the logical anomaly from the deck matrix.</p>
          </div>

          {!hallucinationFound ? (
            <div className="space-y-3">
              <div className="text-xs text-zinc-400 bg-zinc-900/40 border border-zinc-800 p-2 text-center rounded">
                {selectedCard !== null ? `SUSPECTED ANOMALY: PROPHECY_0${selectedCard + 1}` : "SELECT A TARGET PROPHECY CARD"}
              </div>
              <button
                onClick={verifyHallucination}
                disabled={selectedCard === null}
                className="w-full bg-purple-950/40 border border-purple-800/60 text-purple-300 text-xs font-bold py-2 rounded uppercase tracking-wider hover:bg-purple-900/40 transition-colors disabled:opacity-40"
              >
                SUBMIT SUSPECT MARK
              </button>
            </div>
            ...
            <form onSubmit={verifyFinalWord} className="space-y-3">
              <input
                ...
                className="w-full bg-zinc-900/80 border border-purple-900/50 rounded px-3 py-2 text-xs uppercase tracking-widest text-pink-400 focus:outline-none focus:border-pink-500"
              />
              <button
                type="submit"
                disabled={statusState === "SUCCESS"}
                className="w-full bg-pink-950/20 border border-pink-800/40 text-pink-400 text-xs font-bold py-2 rounded uppercase tracking-wider hover:bg-pink-900/20"
              >
                PURGE CONTRADICTION
              </button>
            </form>
        ...
        <div className="bg-zinc-950/60 border border-purple-950 p-5 rounded-xl flex flex-col justify-between h-full min-h-[220px] space-y-4">
          ...
          <button
            ...
            className="w-full bg-zinc-900 hover:bg-zinc-850 border border-purple-950 text-zinc-400 text-[10px] font-bold uppercase py-2 rounded transition-colors"
          >
