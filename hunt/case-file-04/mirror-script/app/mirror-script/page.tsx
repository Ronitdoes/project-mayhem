...
export default function MirrorScriptPage() {
...
// O/p : carnival 17`;
  ...
  const mirroredCode = codeSnippet.split('\n').map(line => {
  ...
  return (
    <div className="min-h-screen bg-black text-red-600 p-8 font-mono flex flex-col items-center justify-center space-y-8">
      <div className="max-w-2xl w-full border-2 border-red-900 p-6 rounded-lg bg-zinc-900 shadow-[0_0_20px_rgba(153,27,27,0.4)]">
        ...
        <form onSubmit={handleSubmit} className="mt-12 flex flex-col items-center space-y-4">
          <input
            type="text"
            value={answer}
            onChange={(e) => setAnswer(e.target.value)}
            placeholder="Enter the output word..."
            className="bg-zinc-800 border border-red-900 text-red-400 p-3 rounded w-full max-w-xs text-center outline-none focus:ring-2 focus:ring-red-600 transition-all"
          />
          <button 
            type="submit" 
            className="bg-red-900 text-white px-6 py-2 rounded hover:bg-red-700 transition-colors uppercase font-bold tracking-tighter"
          >
            Submit
          </button>
        </form>
        ...
        {submitted && (
          <div className={`mt-6 text-center font-bold ${isCorrect ? 'text-green-500' : 'text-red-600'}`}>
            {isCorrect ? 'ACCESS GRANTED: Fragment Restored' : 'ACCESS DENIED: Incorrect Reflection'}
          </div>
        )}
      </div>
    </div>
