...
import { GondolaState } from "@/hooks/useWheelEngine";
import { NUM_GONDOLAS } from "@/lib/constants";
        ...
        {gondolas.map((gondola) => {
          ...
          return (
            <g
              ...
              onClick={() => onGondolaClick(gondola.index)}
              role="button"
              aria-label={`Gondola ${gondola.index}, value ${gondola.displayedNumber}`}
              tabIndex={0}
              onKeyDown={(e) =>
                e.key === "Enter" && onGondolaClick(gondola.index)
              }
            >
              {/* Suspension wire */}
              <line
