...
const LOG_DATA: Packet[] = [
  ...
  { id: "PKT-04", timestamp: "02:44:12.153", sourceIP: "192.168.1.105", destPort: 443, protocol: "HTTPS", sequence: 9942, flags: "PUSH", isAnomaly: true }, // The Intruder
          ...
          <div className="w-full max-w-2xl bg-zinc-950 border border-purple-900/50 rounded-lg p-5 relative shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
              <div>
                ...
                <span className="text-purple-300 font-bold">{selectedPacket.timestamp}</span>
              </div>
              <div>
                <span className="text-zinc-500 block mb-1">SOURCE IP</span>
                <span className="text-purple-300 font-bold">{selectedPacket.sourceIP}</span>
              </div>
              <div>
                <span className="text-zinc-500 block mb-1">DEST PORT</span>
                <span className="text-purple-300 font-bold">{selectedPacket.destPort}</span>
              </div>
              <div>
                <span className="text-zinc-500 block mb-1">PROTOCOL</span>
                <span className="text-purple-300 font-bold">{selectedPacket.protocol}</span>
              </div>
              <div>
                <span className="text-zinc-500 block mb-1">SEQ NUMBER</span>
                <span className="text-purple-300 font-bold">{selectedPacket.sequence}</span>
              </div>
              <div>
                <span className="text-zinc-500 block mb-1">TCP FLAGS</span>
                <span className="text-purple-300 font-bold">{selectedPacket.flags}</span>
              </div>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1.5 bg-purple-950/50 hover:bg-purple-900/50 border border-purple-800 text-[11px] font-bold text-purple-300 rounded transition-all active:scale-95"
              >
                NEXT HINT
              </button>
              {!showSourceHint && (
                <button
                  ...
                  className="px-3 py-1.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-700 text-[11px] font-bold text-zinc-300 rounded transition-all active:scale-95"
                >
                  DECRYPT SIGNATURES
                </button>
              )}
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={verifyPuzzleAnswer} className="space-y-3">
              <input
                ...
                className="w-full bg-zinc-950 border border-zinc-800 focus:border-purple-500 rounded p-2 text-xs text-zinc-200 outline-none transition-colors placeholder:text-zinc-600"
              />
              <button
                type="submit"
                className="w-full py-2 bg-purple-600 hover:bg-purple-500 text-xs font-bold text-white rounded transition-colors active:scale-[0.98]"
              >
                ANALYZE & SUBMIT
              </button>
            </form>
